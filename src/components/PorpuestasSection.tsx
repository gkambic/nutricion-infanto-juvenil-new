...
import PropuestasCard from './PropuestasCard';
...
const PropuestasSection: React.FC<PropuestasSectionProps> = ({ cardsData }) => {
  return (
    <section className="propuestas-section">
      {cardsData.map((card, index) => (
        <PropuestasCard key={index} {...card} />
      ))}
    </section>
  );
};
...
/* 
const PropuestasSection: React.FC = () => {
  const cardsData = [
    {
    ...
    {
      imageUrl: 'https://nutricioninfantojuvenil.com.ar/wp-content/uploads/2024/03/Nutriplanner-2.jpg',
      title: 'Planificación de menús: Nutriplanner',
      description: 'Herramienta práctica para organizar tus comidas semanales de manera eficiente y saludable.',
      listItems: [
        'Incluye plantillas personalizables.',
        'Recetas balanceadas.',
        'Tips para optimizar tus compras y aprovechar mejor tus ingredientes.',
      ],
      buttonText: 'Consultar',
      buttonLink: '#contacto',
      isReversed: true, // Imagen a la izquierda
